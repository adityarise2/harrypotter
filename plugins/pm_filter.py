...
async def season_search(client: Client, query: CallbackQuery):
  ...
  #  del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>" if settings["auto_delete"] else ''
    ...
    if n_offset== '':
        btn.append(
            [InlineKeyboardButton(text="🚸 ɴᴏ ᴍᴏʀᴇ ᴘᴀɢᴇs 🚸", url=f"https://t.me/aj_adminn_bot")]
        )
    elif n_offset == 0:
        btn.append(
    ...
    # If the user isn't an admin, return
      ...
      await client.edit_message_media(
       ...
       await client.edit_message_media(
        ...
        buttons = [[
            InlineKeyboardButton('Repo', url='t.me/ajcinemassofcl')
        ],[
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("📲 ᴄᴏɴᴛᴀᴄᴛ ᴛᴏ ᴏᴡɴᴇʀ ", user_id = ADMINS[1])
               ],[
                    InlineKeyboardButton("⇋ ʙᴀᴄᴋ ⇋", callback_data="about")
                  ]]
            reply_markup = InlineKeyboardMarkup(btn)
